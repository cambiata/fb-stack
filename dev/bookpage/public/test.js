...
var Main = function() {
	...
	console.log("src/Main.hx:21:",BookModel.instance.book);
	...
	haxe_Timer.delay(function() {
		...
		console.log("src/Main.hx:28:","QSyntaxRenderer loaded");
		m.redraw();
		return;
	},3000);
};
...
Main.getVidewView = function() {
	return [m.m("div.specialchapter.video",m.m("h1","Video")),m.m("video",{ config : function(el,isInit,context) {
		context.retain = true;
	}, src : "/assets/video/tada.mp4", controls : true, style : { width : "100%", objectFit : "contain", backgroundColor : "black"}})];
};
Main.prototype = {
	__class__: Main
};
...
Index.prototype = {
	view: function() {
		...
		return [m.m("div.centered",[new BookPage2().view()])];
	}
	,__class__: Index
};
var BookPage2 = function() {
	this.syntaxRenderers = [new markdown_inlineRenderer_QInlineSyntaxRenderer()];
};
$hxClasses["BookPage2"] = BookPage2;
BookPage2.__name__ = ["BookPage2"];
BookPage2.__interfaces__ = [mithril_Mithril];
BookPage2.prototype = {
	view: function() {
		if(arguments.length > 0 && arguments[0].tag != this) return arguments[0].tag.view.apply(arguments[0].tag, arguments);
		return [m.m("#bookpage",[this.header(),this.left(),this.bookContent()])];
	}
	,header: function() {
		return m.m("#bookheader","BookHeader");
	}
	,left: function() {
		var _gthis = this;
		return m.m("#bookleft",[m.m("#booktoc",BookModel.instance.book.chapters.map(function(ch) {
			return m.m("button.#booktocitem" + (_gthis.getSelectedChapter() == ch ? ".selected" : ""),{ onclick : function(e) {
				BookModel.instance.set_chapter(ch);
				BookModel.instance.set_subchapter(null);
				m.redraw();
				return;
			}},ch.title);
		}))]);
	...
	,bookContent: function() {
		return m.m("#bookcontent",[this.getChapterMarkup(),this.getSubchapterNav(),this.getSubchapterMarkup()]);
	}
	,getChapterMarkup: function() {
		var _gthis = this;
		var id = this.getSelectedChapter().id;
		var md = this.getSelectedChapter().text;
		var tmp = m.m("textarea",{ value : md, oninput : function(e) {
			markdown_MithrilRendererCache.instance.clearCacheKey(id);
			return _gthis.getSelectedChapter().set_text(e.target.value);
		}, style : { width : "90%", height : "140px"}});
		return [m.m("details",{ open : false},[tmp]),this.getChaptertype(this.getSelectedChapter()),markdown_MithrilRendererCache.instance.markdownToView(id,md,this.syntaxRenderers)];
	}
	,getSubchapterMarkup: function() {
		var _gthis = this;
		var id = this.getSelectedChapter().id + "/" + this.getSelectedSubchapter().id;
		var md = this.getSelectedSubchapter().text;
		var tmp = m.m("textarea",{ value : md, oninput : function(e) {
			markdown_MithrilRendererCache.instance.clearCacheKey(id);
			return _gthis.getSelectedSubchapter().set_text(e.target.value);
		}, style : { width : "90%", height : "140px"}});
		return [m.m("details",{ open : false},[tmp]),this.getChaptertype(this.getSelectedSubchapter()),markdown_MithrilRendererCache.instance.markdownToView(id,md,this.syntaxRenderers)];
	}
	,getChaptertype: function(chapter) {
		var type = chapter.type;
		if((type == null ? null : js_Boot.getClass(type)) == data_VideoChaptertype) {
			return new ui_VideoChapter(type).view();
		} else {
			return m.m("div#chaptertype","" + Std.string(chapter.type));
		}
	}
	,getSubchapterNav: function() {
		var _gthis = this;
		return m.m("#chaptertoc",this.getSelectedChapter().subchapters.map(function(sub) {
			return m.m("button.#chaptertocitem" + (_gthis.getSelectedSubchapter() == sub ? ".selected" : ""),{ onclick : function(e) {
	...
	,getSelectedChapter: function() {
		try {
			...
			return BookModel.instance.book.chapters[0];
		} catch( e ) {
			console.log("src/Main.hx:163:","error: " + Std.string((e instanceof js__$Boot_HaxeError) ? e.val : e));
			return null;
		}
	}
	,getSelectedSubchapter: function() {
		try {
			if(BookModel.instance.subchapter != null) {
				return BookModel.instance.subchapter;
			}
			return this.getSelectedChapter().subchapters[0];
		} catch( e ) {
			console.log("src/Main.hx:175:","error: " + Std.string((e instanceof js__$Boot_HaxeError) ? e.val : e));
			return null;
		}
	}
	,__class__: BookPage2
...
data_ContentFilters.fiterRoomAndShelfHome = function(ct) {
	return new data_Content({ id : ct.id, rooms : ct.rooms.filter(function(room) {
		...
		return new data_Room({ id : room1.id, title : room1.title, shelves : room1.shelves.filter(function(shelf) {
			console.log("../../src/data/Content.hx:189:",shelf.id);
...
data_ContentUtils.randomChapters = function(bookId,count) {
	...
	return _g.map(function(i) {
		...
		var type = data_ContentUtils.typeCounter++ % 5 == 2 ? new data_VideoChaptertype({ url : "/assets/video/tada.mp4"}) : new data_StandardChaptertype();
...
markdown_MithrilRendererCache.prototype = {
	...
	,markdownToView: function(id,md,syntaxRenderers) {
		...
		if(__map_reserved[id] != null ? _this.existsReserved(id) : _this.h.hasOwnProperty(id)) {
			console.log("../../src/markdown/MithrilRendererCache.hx:30:","get " + id + " from cache!");
...
var ui_VideoChapter = function(c) {
	this.c = c;
	console.log("../../src/ui/VideoChapter.hx:9:","VIDEO CHAPTER CREATED");
};
