...
function nameToUrl (name, sf, format) {
  ...
  return '/midi-js-soundfonts/' + sf + '/' + name + '-' + format + '.js'
}
...
module.exports = function (url, type) {
  return new Promise(function (done, reject) {
    ...
    req.open('GET', url)
    req.onload = function () {
      req.status === 200 ? done(req.response) : reject(Error(url + ':' + req.statusText))
    }
    req.onerror = function () { reject(Error('Network Error ' + url)) }
    req.send()
  })
}
