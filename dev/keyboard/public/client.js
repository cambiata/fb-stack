...
Main.__name__ = ["Main"];
Main.main = function() {
	var appState = { text : "Server", keysActive : null, keyboard : { lowestKey : 40, highestKey : 62}, keysHover : [], keysPressed : []};
...
Index.prototype = {
	view: function() {
		...
		return [m.m("div","" + Std.string(this.appState)),new web_view_KeyboardRandom(this.appState).view(),m.m("button",{ onclick : function(e) {
			var pitch = audio_waa_PitchRecognizer.instance == null ? audio_waa_PitchRecognizer.instance = new audio_waa_PitchRecognizer(null) : audio_waa_PitchRecognizer.instance;
			pitch.onPitch = $bind(_gthis,_gthis.onPitch);
	...
	,onPitch: function(currentFreq,closestIndex,maxVolume) {
		console.log("src/Main.hx:83:",closestIndex);
...
audio_waa_PitchRecognizer.prototype = {
	onPitchHandler: function(currentFreq,closestIndex,maxVolume) {
		if(this.onPitch != null) {
			...
			console.log("../../cx/src/audio/waa/PitchRecognizer.hx:51:",[currentFreq,closestIndex,maxVolume]);
		}
	}
	...
	stretch: function(amount) {
		...
		while(seedrest > 0) {
			...
			while(_g1 < columns.length) {
				...
				var threshold = seedThreshold_h[valueDeltaInt];
...
nx3_PBaseRectCalculator.prototype = {
	getBaseRect: function() {
		...
		switch(_g._hx_index) {
		case 0:
			var atr = _g.attributes;
			var a = _g.articulations;
			var v = _g.variant;
			var h = _g.heads;
	...
	getTieRects: function() {
		...
		while(_g < _g1.length) {
			...
			if(head.nhead.tie != null) {
				...
				if(_g2._hx_index == 0) {
					var tlevel = _g2.level;
					var tiedir = _g2.direction;
					tielevel = tlevel;
				}
...
var nx3_PNoteHeadsRectTplCalculator = function(note) {
	...
	case 2:
		var octave = _g1.octave;
		var modus = _g1.modus;
		tmp = 0;
		break;
	case 3:
		var octave1 = _g1.octave;
		var modus1 = _g1.modus;
		tmp = level * 3;
		break;
	default:
		tmp = 0;
...
nx3_PPart.prototype = {
	...
	,getRect: function() {
		...
		var result = _g._hx_index == 0 ? new nx3_geom_Rectangle(0,-8,1,13) : new nx3_geom_Rectangle(0,-4,1,8);
		var _g1 = 0;
...
nx3_PScore.prototype = {
	...
	,getSystems: function(systemwidth) {
		...
		while(_g2 < _g11.length) {
			...
			var ifMoreThan = system1 != array[array.length - 1] ? 0 : system1.getSystemBreakWidth() * .7;
...
nx3_PStaveRectCalculator.prototype = {
	getStaveRect: function() {
		...
		var headw = _g._hx_index == 2 ? 2.2 : 1.6;
		var rect = null;
...
nx3_PSystem.prototype = {
	...
	,getSpaceAbovePart: function(partIdx) {
		...
		while(_g < _g1.length) {
			...
			if(part == null) {
				console.log("../../cx/src/nx3/PSystem.hx:71:","part == null");
				continue;
			}
...
nx3_PVoiceBeamgroupsGenerator.prototype = {
	...
	,getNotesBeamgroupPosIndexes: function(patternPositions,notesPositions) {
		...
		while(_g11 < _g2) {
			...
			case 0:
				var attributes = _g21.attributes;
				var articluation = _g21.articulations;
				var variant = _g21.variant;
				var heads = _g21.heads;
				patternIdx = nx3_ENoteValTools.beaminglevel(nnote.value) <= 0 ? -1 : findPatternIdxForNote(curNotePos1);
				break;
			case 1:
				var level = _g21.level;
				patternIdx = -1;
				break;
			default:
				patternIdx = -1;
	...
	,drawNoteHeads: function(system,systembar,note) {
		...
		case 3:
			...
			if(pause) {
				...
				console.log("../../cx/src/nx3/render/Renderer.hx:581:",numSvgStr);
			}
...
nx3_render_TargetSvgXml.prototype = {
	...
	,polyfill: function(x,y,coordinates,lineWidth,lineColor,fillColor) {
		...
		console.log("../../cx/src/nx3/render/TargetSvgXml.hx:237:","Polyfill: " + x + " " + y);
		...
		console.log("../../cx/src/nx3/render/TargetSvgXml.hx:255:",path);
		var el = Xml.createElement("path");
		el.set("d",path);
		el.set("fill","#000");
		el.set("stroke","#000");
...
web_view_Keyboard.__interfaces__ = [mithril_Mithril];
web_view_Keyboard.prototype = {
	getKeyboard: function() {
		...
		var _g2 = highestOctave;
		while(_g11 < _g2) {
			var oct = _g11++;
			var keyboard = new web_view_KeyboarOctaveExt(this.appState,oct,excludeKeys,null,null,null,null,web_view_Keyboard.octaveLabels[oct]);
			keybOctaves.push(keyboard);
		}
		return keybOctaves;
	}
	,view: function() {
...
web_view_KeyboardOctave.__interfaces__ = [mithril_Mithril];
web_view_KeyboardOctave.prototype = {
	...
	,getOctave: function() {
		...
		while(_g < 12) {
			...
			var keyExclude = this.excludeKeyNumbers.indexOf(keyNumber[0]) != -1 ? ".key-exclude" : "";
			keys.push(m.m("div.key" + keyType + keyExclude,{ style : st, onmousedown : (function(shift1,keyNumber1) {
				return function(e) {
					_gthis.onMouseDown(keyNumber1[0] + shift1[0]);
				};
			})(shift,keyNumber), onmouseup : (function(shift2,keyNumber2) {
				return function(e1) {
					_gthis.onMouseUp(keyNumber2[0] + shift2[0]);
				};
			})(shift,keyNumber), onmouseover : (function(shift3,keyNumber3) {
				return function(e2) {
					_gthis.onMouseOver(keyNumber3[0] + shift3[0]);
				};
			})(shift,keyNumber), onmouseout : (function(shift4,keyNumber4) {
				return function(e3) {
					_gthis.onMouseOut(keyNumber4[0] + shift4[0]);
				};
...
web_view_KeyboardRandom.__interfaces__ = [mithril_Mithril];
...
web_view_SingleNote.__interfaces__ = [mithril_Mithril];
web_view_SingleNote.prototype = {
	view: function() {
		if(arguments.length > 0 && arguments[0].tag != this) return arguments[0].tag.view.apply(arguments[0].tag, arguments);
		var svgStr = "";
		var firstkey = this.state.keysHover.length > 0 ? this.state.keysHover[0] : 0;
		return [this.state.keysHover.length > 0 ? m.m("div.svg",m.trust(web_view_SingleNoteCache.getSvg(firstkey))) : m.m("div","-")];
	}
...
web_view_SingleNoteCache.createNotes = function(lowestNr,highestNr) {
	...
	while(_g1 < _g) {
		...
		while(_g2 < cleafsAndHeads.length) {
			...
			var bar = new nx3_NBar([new nx3_NPart([new nx3_NVoice(notes)],null,clef,null,key),new nx3_NPart([new nx3_NVoice(namenotes)],nx3_EPartType.Lyrics)],null,null,null,nx3_EAllotment.Equal);
			bars.push(bar);
		}
		var score = new nx3_NScore(bars);
		var pscore = new nx3_PScore(score);
		var target = new nx3_render_TargetSvgXml("svg",nx3_render_scaling_Scaling.NORMAL);
		var renderer = new nx3_render_Renderer(target,0,0);
		renderer.renderScore(pscore,0,0,400);
		var svg = target.getXml();
		var svgStr = haxe_xml_Printer.print(svg);
...
web_view_SingleNoteCache.noteToHead = function(noteNr) {
	...
	var getHeads = function(noteNr1,clef) {
		...
		var noteInOctave = noteNr1 % 12;
		var level = -7 * (octave - 4);
		if(clef == nx3_EClef.ClefF) {
			level += -12;
		}
		console.log("../../cx/src/web/view/SingleNote.hx:100:",octave);
		var heads;
		switch(noteInOctave) {
		case 0:
	...
	var getNames = function(noteNr2) {
		var noteInOctave1 = noteNr2 % 12;
		var names;
		switch(noteInOctave1) {
		case 0:
			names = ["d"];
			break;
		case 1:
