...
import nx3.render.TargetPdfkit;
...
{	
	 ...
	 public function flashBridge(data:Dynamic) {
		 trace('flashBridge success! data: ' + data);
		var bytes:Bytes = createMidiFile();		
		var bytesData:BytesData = bytes.getData();
		cx.JSTools.saveFile(bytesData, 'Test-MIDI.midi');
	 }		
	
	 public function new() {
		var ta:TextAreaElement = cast Browser.document.getElementById('ta');
		
		var btn = Browser.document.getElementById('btn');
		btn.addEventListener('click', function(e) {
			//new PdfDoc();
			
			var qscore = ta.value.trim();
			
			var parser = new nx3.qs.QSParser(qscore);
			var nscore = parser.parse();
			var target = new TargetPdfkit();
			var renderer = new nx3.render.Renderer(target, 50, 50);			
		...
		var svgDiv:DivElement =  cast Browser.document.getElementById('svgDiv');
		ta.addEventListener('keyup', function(e) {
			var qscore = ta.value.trim();
			if (prevQscore == qscore) return;
			prevQscore = qscore;			
			
			var parser = new nx3.qs.QSParser(qscore);
			var nscore = parser.parse();
			var target = new nx3.render.TargetSvgXml('test');
			var renderer = new nx3.render.Renderer(target);
			renderer.renderScore(new nx3.PScore(nscore));
			var svg = target.getXml().toString();					
			svgDiv.innerHTML = svg;
			
		});		 
		 
	 };
	static private var prevQscore:String;
...
class PdfDoc {
	...
	public function new() {
		...
		this.iframe = js.Browser.document.getElementById('iframe');
	...
	function after() 
	{
		...
		#else
			//var blobStream = untyped js.Browser.window.blobStream();
...
extern class PDFDocument {
	function new(); 
	function end():Void;
	function pipe(dest:Dynamic, ?options:Dynamic):Dynamic;
	
	function fontSize(size:Float):PDFDocument;
	
	@:overload(function(text:String, options:Dynamic):PDFDocument{})
	function text(text:String, x:Float, y:Float, ?options:Dynamic):PDFDocument;
	
	function save():PDFDocument;
	function moveTo(x:Float, y:Float):PDFDocument;
	function lineTo(x:Float, y:Float):PDFDocument;
	function fill(fill:String, ?rule:Dynamic):PDFDocument;
	
	function circle(x:Float, y:Float, radius:Float):PDFDocument;
	
	function scale(xFactor:Float, ?yFactor:Float, ?options:Dynamic):PDFDocument;
	function translate(x:Float, y:Float):PDFDocument ;
	function path(path:String):PDFDocument;
	function restore():PDFDocument;
	
	function moveDown(?lines:Int):PDFDocument;
	function moveUp(?lines:Int):PDFDocument;
	function font(family:String, size:Float):PDFDocument;
}
*/
