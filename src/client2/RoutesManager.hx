...
class RoutesManager {
	...
	public static var instance(default, null):RoutesManager = new RoutesManager();
	...
	public function init() {
		...
		M.route(js.Browser.document.querySelector('#routes'), '/', routes);
	}
	...
	public var contentHandler:SimpleRouteResolver = {
		onmatch: function(args, path) {
			try {
				ErrorsAndLogs.addLog('RouteResolver:$path: ' + args + '');
				FilterManager.instance.setFilterContent(cast args);
			} catch (e:Dynamic) {
	...
	public var shelvesHandler:SimpleRouteResolver = {
		onmatch: function(args, path) {
			try {
				ErrorsAndLogs.addLog('RouteResolver:$path: ' + args + '');
				FilterManager.instance.setFilterContent(cast args);
				SidewaysManager.instance.pageIdx = 1;
			} catch (e:Dynamic) {
				ErrorsAndLogs.addError('RouteResolver roomHandler Error: $e');
				null;
			}
	...
	public var homeHandler:SimpleRouteResolver = {
		onmatch: function(args, path) {
			try {
				ErrorsAndLogs.addLog('RouteResolver:$path: ' + args + '');
				FilterManager.instance.setFilterContent(null);
			} catch (e:Dynamic) {
				ErrorsAndLogs.addError('RouteResolver roomHandler Error: $e');
				null;
			}
