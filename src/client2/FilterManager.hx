...
using FilterManager.FilterTools;
using cx.ArrayTools;
...
class FilterManager {
	...
	public function setFilterContent(ref:ContentRef) {
		...
		SidewaysManager.instance.setPageFromContentRef(ref);
		M.redraw();
	}

	public function getRoom():Room {
		return try {
			ContentManager.instance.content.rooms.filter(room -> room.id == this.filterContent.roomId).first();
		} catch (e:Dynamic) {
			...
			ContentManager.instance.content.rooms.first();
		}
	}
