...
class QSParser {
	...
	public function new(str:String) {
		init();		
		str = cx.StrTools.replaceAll(str, '  ', ' ');		
		str = QSParserTools.urlDecode(str);		
		//trace('Decoded qscode: $str');
		this.str = str;
	}
		...
		for (b in matrix) {
			...
			if (partLength != firstLenght) {
				while (partLength < firstLenght) {
					...
					partLength++;
					trace('Inconsistent part numbers: $firstLenght $partLength $str');
				}
				return false;
			}
		}
...
class QSParserTools {
	static public var relaceSlash = '---';
	static public function urlEncode(qcode:String): String {
		return qcode.replace('/', relaceSlash);		
	}
	
	static public function urlDecode(qcode:String):String {
		return qcode.replace(relaceSlash, '/');
	}
	
	static public function filterLyrics(qcode:String):String {
		var pos = qcode.indexOf('/lyr');
		if (qcode.indexOf('/lyr') > -1) {
			qcode = qcode.substr(pos + 4);
		}
		return qcode;
	}
