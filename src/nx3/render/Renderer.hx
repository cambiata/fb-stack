...
class Renderer
{
	...
	public function drawBarAttributeTime(system:PSystem, systembar:PSystemBar, part:PPart /*, nx:Float, ny:Float*//*, timeX:Float=0*/)
	{
		...
		if (timeChars.length == 2)
		{
			...
			this.target.shape(timeChars.first(), tx + timeX, ty + timeY +  part_getYPosition * this.scaling.unitY, upperXmlStr);		
			
			var lowerXmlStr = getSvgNumber(timeChars.second());			
			var timeY = 1 * this.scaling.unitY;					
			this.target.shape(timeChars.second(), tx + timeX, ty + timeY +  part_getYPosition * this.scaling.unitY, lowerXmlStr);		
		}
		else
		{
			var midXmlStr = getSvgNumber(timeChars.first());
			var timeY = -1 * this.scaling.unitY;					
			this.target.shape(timeChars.first(), tx + timeX, ty + timeY +  part_getYPosition * this.scaling.unitY, midXmlStr);					
		}
	}
	...
	public function drawBarAttributeClef(system:PSystem, systembar:PSystemBar, part:PPart/*, nx:Float, ny:Float *//*,  clefX:Float=0*/)
	{
		...
		this.target.shape(actclef.getName(), tx + clefX, ty + clefY +  part_getYPosition * this.scaling.unitY, svgXmlstr);		
	}
	...
	public function drawNoteHeads(system:PSystem, systembar:PSystemBar, note:PNote):Void 
	{
		...
		{			
			...
			default:
				...
				//var tag = note.nnote.value.head().getName();
		...
		{	
			...
			if (firstnote.nnote.value.beaminglevel() > 0)
			{
				if (beamgroup.getDirection() == EDirectionUD.Up)
				{
					...
					var flagTag = (firstnote.nnote.value.beaminglevel() == 2) ? 'Flag16Up' : 'Flag8Up';
					this.target.shape(flagTag, tx + leftX - adjustX , rightY + adjustY +leftTipY, flag, 0x000000);
				}
				else
				{
					var adjustX = 0.6 * scaling.unitX;
					...
					var flagTag = (firstnote.nnote.value.beaminglevel() == 2) ? 'Flag16Down' : 'Flag8Down';
					this.target.shape(flagTag, tx  + leftX- adjustX , rightY + adjustY +leftTipY,flag, 0x000000);
				}
			}
		}		
