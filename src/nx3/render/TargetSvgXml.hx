...
class TargetSvgXml implements ITarget {
	...
	public function filledellipse(x:Float, y:Float, rect:Rectangle, ?lineWidth:Float, ?lineColor:Int, ?fillColor:Int):Void {
		...
		r.set('style', 'fill: #000; stroke: #000; stroke-width: ${lineWidth * scaling.linesWidth}; cursor:pointer;');
	...
	public function shape(?shapeId:String, x:Float, y:Float, xmlStr:String, ?fillColor:Int = 0x000000):Void {
		...
		if (elementTag == 'path') {
			...
			throw 'Shape element $elementTag - UNIMPLEMENTED';
	...
	public function polyfill(x:Float, y:Float, coordinates:Pnts, ?lineWidth:Float = 1, ?lineColor:Int = 0x000000, fillColor:Int = 0x0000FF):Void {
		...
		var path = 'M ' + Std.string((coord.x * scaling.unitX) + x) + ' ' + Std.string((coord.y * scaling.unitY) + y) + ' ';

		for (coord in coordinates) {
			path += 'L ' + Std.string((coord.x * scaling.unitX) + x) + ' ' + Std.string((coord.y * scaling.unitY) + y) + ' ';
		}
	...
	/* INTERFACE nx3.render.ITarget */
	public function interactiveEllipse(x:Float, y:Float, rect:Rectangle, ?lineWidth:Float, ?lineColor:Int, ?fillColor:Int, cb:EActivityType->
		Void = null):Void {}
	...
	#else
	public function textwidth(text:String):Float {
		return this.font.size * text.length;
	}
	...
	#end
	// static inline function hex(int:Int):String	return (int == 0) ? "#000" : "#" + cx.StringTools.hex(int);
	#if (js)
