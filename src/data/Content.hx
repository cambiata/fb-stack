...
class VideoChaptertype implements IChaptertype {
	public var url:String = '/test/Vinjett.mp4';
}
...
class ScorxmixerChaptertype implements IChaptertype {
	// public var filename:String = '/test.pdf';
	public var paths:Array<String> = ['/scorx/_test/200.mp3', '/scorx/_test/100.mp3',];
}
...
class ContentFilters {
	...
	static public function fiterRoomAndShelfHome(ct:Content) {
		return new Content({
			id: ct.id,
			rooms: ct.rooms.filter(room -> room.id == 'home').map(room -> {
				new Room({
					id: room.id,
					title: room.title,
					shelves: room.shelves.filter(shelf -> {
						trace(shelf.id);
						shelf.id == 'home';
					})
				});
	...
	static public function filterAccess(ct:Content, access:Int = 0) {
		return new Content({
			id: ct.id,
			rooms: ct.rooms.map(room -> {
				new Room({
					id: room.id,
					shelves: room.shelves.filter(shelf -> shelf.access <= access).map(shelf -> {
						new Shelf({
							id: shelf.id,
							title: shelf.title,
							info: '' + shelf.info,
							access: shelf.access,
							books: shelf.books.filter(book -> book.access <= access).map(book -> {
								new Book({
									id: book.id,
									title: book.title,
									access: book.access,
									type: book.type,
									info: '' + book.info,
									chapters: book.chapters.filter(chapter -> chapter.access <= access).map(chapter -> {
										new Chapter({
											id: chapter.id,
											title: chapter.title,
											...
											info: chapter.info,
											text: chapter.text,
											subchapters: chapter.subchapters.filter(s -> s.access <= access)
										});
									})
								});
							})
	...
	static public function sort(ct:Content):Content {
		ct.rooms.sortA((a, b) -> a.sort - b.sort).iter(room -> room.shelves.sortA((a, b) -> a.sort - b.sort).iter(shelf -> shelf.books.sortA((a, b) -> a
			.sort - b.sort)));
		return ct;
	}
}
