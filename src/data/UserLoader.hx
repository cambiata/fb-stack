...
class UserLoader {
	...
	public function startSession() {
		...
		return UserLoader.instance.getCurrentBrowserUser().then(browserUser -> {
			if (browserUser == null) {
		...
		haxe.Timer.delay(() -> {
			firebase.Firebase.app().auth().onAuthStateChanged(user -> {
				if (user != null) {
					...
					if (user.email == UserModel.instance.userState.toData().email)
						return null;
	...
	public function load() {
		firebase.Firebase.app().auth().onAuthStateChanged(user -> {
			if (user != null) {
				...
				UserModel.instance.userState = Loading;
				ApiCalls.getAuthRequest('/api/userconfig').then(data -> {
					// UserModel.instance.setLoadedUserFromData(data);
					var d:Dynamic = data;
					trace('------------------------------------');
					trace('UserModelLoaded');
